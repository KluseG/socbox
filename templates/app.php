<!DOCTYPE html>
<html lang="en">
  <head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title>(3) SocBox</title>

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta/css/bootstrap.min.css" integrity="sha384-/Y6pD6FV/Vv2HJnA6t+vslU6fwYXjCFtcEpHbNJ0lyAFsXTsjBbfaDjzALeQsN6M" crossorigin="anonymous">
    <link rel="stylesheet" href="/assets/css/app.css">
    <link href="https://fonts.googleapis.com/icon?family=Material+Icons"
      rel="stylesheet">
  </head>
  <body>
    <div class="col top-col">
      <div class="top-bar">
        <div class="row">
          <div class="col d-lg-flex col-lg-4" onclick="window.location.href='/'" style="cursor: pointer">
            <span class="logo">SocBox</span>
          </div>
          <div class="col d-lg-flex col-lg-4 justify-content-center">
            <form method="post" action="/search">
              <div class="input-group">
                <input type="text" class="form-control" id="search" name="string" placeholder="Search SocBox">
                <div class="input-group-addon">
                  <button type="submit" class="btn"><i class="material-icons">search</i></button>
                </div>
              </div>
            </form>
          </div>
          <div class="col col-lg-4 d-lg-flex justify-content-end profile-ddown">
            <div class="top-notifications">
              <div class="dropdown">
                <a href="#" class="dropdown-toggle has-notify" role="button" id="notificationDropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="material-icons">notifications_active</i><span class="notifications-count">3</span></a>

                <div class="dropdown-menu" aria-labelledby="notificationDropdown">
                  <a class="dropdown-item" href="#">
                    <div class="media">
                      <img class="d-flex align-self-start mr-3" src="https://sslb.ulximg.com/image/405x405/artist/1346353449_4159240d68a922ee4ecdfd8e85d179c6.jpg/e96a72d63f272127d0b6d70c76fd3f61/1346353449_eminem.jpg" alt="Generic placeholder image">
                      <div class="media-body">
                        <h6 class="mt-0">Eminem <small class="nickname">@iameminem</small> <span class="date-ago text-right">15m</span></h6>
                        <p>is now following you!</p>
                      </div>
                    </div>
                  </a>
                  <a class="dropdown-item" href="#">
                    <div class="media">
                      <img class="d-flex align-self-start mr-3" src="https://media.nngroup.com/media/people/photos/gibbons_sarah-800px.jpg.400x400_q95_autocrop_crop_upscale.jpg" alt="Generic placeholder image">
                      <div class="media-body">
                        <h6 class="mt-0">Sarah Anderson <small class="nickname">@saraha</small> <span class="date-ago text-right">12h</span></h6>
                        <p>has recated to your shout!</p>
                      </div>
                    </div>
                  </a>
                  <a class="dropdown-item" href="#">
                    <div class="media">
                      <img class="d-flex align-self-start mr-3" src="data:image/jpeg;base64," alt="Generic placeholder image">
                      <div class="media-body">
                        <h6 class="mt-0">John Doe <small class="nickname">@jjjohnD</small> <span class="date-ago text-right">1w</span></h6>
                        <p>mentioned you!</p>
                      </div>
                    </div>
                  </a>
                </div>
              </div>
            </div>
            <div class="dropdown">
              <div class="media">
                <img class="d-flex align-self-center mr-3" src="/assets/img/{% avatar %}" alt="{% firstName %}'s profile pic">
                <div class="media-body">
                    <a href="#" class="dropdown-toggle" role="button" id="profileDropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{% firstName %} <i class="material-icons">keyboard_arrow_down</i></a>

                    <div class="dropdown-menu" aria-labelledby="profileDropdown">
                      <a class="dropdown-item" href="/user/settings">Settings <i class="material-icons text-right">settings</i></a>
                      <a class="dropdown-item" href="/logout">Sign out <i class="material-icons text-right">arrow_forward</i></a>
                    </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="container-fluid" id="mainWrapper">
      <div class="row">
		      <div class="col col-lg-2 shortcuts-col">
            <div class="col-12 profile d-flex justify-content-center" onclick="window.location.href='/user/profile'" style="cursor: pointer">
              <div class="media">
                <img class="d-flex align-self-center mr-3" src="/assets/img/{% avatar %}" alt="Adrian's profile pic">
                <div class="media-body align-self-center">
                  <h6 class="mt-0">{% firstName %} {% lastName %}</h6>
                </div>
              </div>
            </div>
            <div class="col-12 shortcuts d-flex justify-content-center">
              <div class="shortcuts-wrapper">
                <a href="#"><i class="material-icons mr-3">people</i> Following</a>
                <a href="#"><i class="material-icons mr-3">people_outline</i> Followers</a>
                <a href="#"><i class="material-icons mr-3">chat</i> Published posts</a>
                <a href="#"><i class="material-icons mr-3">thumbs_up_down</i> Rated posts</a>
              </div>
            </div>
          </div>
		      <div class="col col-lg-4 feed-col">
            <div class="col-12 add-feed-box">
             <form class="add-feed-form" id="addFeed" role="form" method="post" action="/post/add">
               <div class="form-group">
                 <textarea class="form-control" id="add-feed-area" rows="1" placeholder="What's on your mind?" name="body" required></textarea>
               </div>
               <div class="add-feed-sub form-group">
                 <div class="d-flex col align-items-center">
                   <a href="#"><i class="material-icons">insert_photo</i> Photo/Video</a>
                   <div class="col d-flex justify-content-end align-items-center">
                     <div class="dropdown">
                       <a href="#" class="dropdown-toggle" role="button" id="privacyDropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="material-icons">done_all</i> <small>Privacy</small></a>
                         <div class="dropdown-menu" aria-labelledby="privacyDropdown">
                           <a class="dropdown-item" href="#"><i class="material-icons">people_outline</i> Followers only</a>
                           <a class="dropdown-item" href="#"><i class="material-icons">done_all</i> Everyone</a>
                         </div>
                     </div>
                     <button type="submit" role="button" class="btn btn-primary">Post</button>
                   </div>
                 </div>
               </div>
             </form>
           </div>
           <?php foreach($posts as $post) : ?>
           <div class="col-12 feed-box">
              <div class="feed-box-group">
                <div class="feed-box-author">
                  <div class="media">
                    <img class="d-flex align-self-start mr-3" src="/assets/img/<?php echo $post['avatar'] ?>" alt="Generic placeholder image">
                    <div class="media-body">
                      <h6 class="mt-0"><?php echo $post['ufname'] ?> <?php echo $post['ulname'] ?> <small class="nickname" onclick="returnUrl(this)" data-href="/user/profile/<?php echo $post['uname'] ?>">@<?php echo $post['uname'] ?></small> <?php if ($post['canEdit']) : ?><a href="#" class="float-right"><i class="material-icons">mode_edit</i></a><?php endif; ?></h6>
                      <span class="date-ago text-right"><?php echo $post['created'] ?></span>
                    </div>
                  </div>
                </div>
                <div class="feed-box-toggler justify-content-center align-items-center">
                  <a href="#" class="close-comment"><i class="material-icons">close</i></a>
                </div>
              </div>
              <div class="feed-box-group">
                <div class="feed-box-content">
                  <p><?php echo $post['body'] ?></p>
                </div>
                <div class="feed-box-util">
                  <p><i class="material-icons">thumb_up</i> <small>0</small> | <i class="material-icons">thumb_down</i> <small>0</small></p>
                  <p class="text-right"><?php echo $post['commentsCount']; ?> Comments</p>
                  <hr>
                  <a href="#" class="showReactionOpions" data-toggle="popover" data-trigger="focus" data-placement="top" data-content="<a href='#' class='rate-thumb-up'><i class='material-icons'>thumb_up</i></a> <small>or</small> <a href='#' class='rate-thumb-down'><i class='material-icons'>thumb_down</i></a>"><i class="material-icons">thumb_up</i> Rate</a> |
                  <a href="#" class="send-comment"><i class="material-icons">chat_bubble</i> Comment</a>
                </div>
                <div class="feed-box-comments">
                  <div class="comment-form-wrapper">
                    <form class="add-comment-form" id="commentForm" method="post" action="/post/comment">
                      <div class="form-group">
                        <input type="hidden" name="postId" value="<?php echo $post['id']; ?>">
                        <textarea class="form-control" rows="1" placeholder="What would you like to say?" name="body"></textarea>
                        <a href="#" class="send-btn comment-submit"><i class="material-icons">send</i></a>
                      </div>
                    </form>
                  </div>
                  <div class="comments">
                    <?php if ($post['comments']) : ?>
                      <?php foreach ($post['comments'] as $comment) : ?>
                        <div class="comment">
                          <div class="media">
                            <img class="d-flex align-self-start mr-3" src="/assets/img/<?php echo $comment['author']['avatar'] ?>" alt="Generic placeholder image">
                            <div class="media-body">
                              <h6 class="mt-0"><?php echo $comment['author']['first_name']; ?> <?php echo $comment['author']['last_name']; ?> <small class="nickname" onclick="returnUrl(this)" data-href="/user/profile/<?php echo $comment['author']['username'] ?>">@<?php echo $comment['author']['username']; ?></small> <span class="date-ago text-right"><?php echo $comment['created_at']; ?></span></h6>
                              <p><?php echo $comment['body']; ?></p>
                            </div>
                          </div>
                        </div>
                      <?php endforeach; ?>
                    <?php endif; ?>
                  </div>
                </div>
              </div>
          </div>
        <?php endforeach; ?>
		     </div>
		       <div class="col col-lg-3 ads-col no-padding">
             <div class="col-12 ads-wrapper no-padding">
               <div class="col-12 ad-box">
                 <div class="ad-box-title">
                   <h6 class="dimmed">TRENDING <small class="float-right"><a href="#" id="trending7">7d</a> <a href="#" id="trending30">30d</a> <a href="#" id="trendingAll"><i class="material-icons">all_inclusive</i></a></small></h6>
                 </div>
                 <div class="ad-box-media">
                 </div>
                 <div class="ad-box-content">
                   <div class="media">
                    <i class="material-icons d-flex align-self-start mr-3">show_chart</i>
                    <div class="media-body">
                      <h6 class="mt-0 hashtag">#newsocialisgreat</h6>
                      <p>3M people talking about this</p>
                    </div>
                  </div>
                  <div class="media">
                   <i class="material-icons d-flex align-self-start mr-3">show_chart</i>
                   <div class="media-body">
                     <h6 class="mt-0 hashtag">#miley</h6>
                     <p>1.2M people talking about this</p>
                   </div>
                 </div>
                 <div class="media">
                  <i class="material-icons d-flex align-self-start mr-3">show_chart</i>
                  <div class="media-body">
                    <h6 class="mt-0 hashtag">#f4f</h6>
                    <p>820k people talking about this</p>
                  </div>
                </div>
                 </div>
               </div>
               <div class="col-12 ad-box">
                 <div class="ad-box-title">
                   <h6 class="dimmed">SPONSORED</h6>
                 </div>
                 <div class="ad-box-media">
                   <img src="https://baremetrics.imgix.net/blog/slack-featured.png" alt="Slack" class="img-fluid">
                 </div>
                 <div class="ad-box-content">
                   <h6>Slack</h6>
                   <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. <a href="#" class="d-block">slack.com</a></p>
                 </div>
                 <hr>
                 <div class="ad-box-media">
                   <img src="https://www.digitalocean.com/assets/media/tutorials/02-fe955a16.png" alt="Digital Ocean" class="img-fluid">
                 </div>
                 <div class="ad-box-content">
                   <h6>Digital Ocean</h6>
                   <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. <a href="#" class="d-block">digitalocean.com</a></p>
                 </div>
               </div>
                <div class="col-12 ad-box">
                  <div class="ad-box-title">
                    <h6 class="dimmed">LANGUAGE</h6>
                  </div>
                  <div class="ad-box-content lang-content">
                    <a href="#" class="selected">English (US)</a><a href="#">Polski</a>
                  </div>
                </div>
             </div>
           </div>
           <div class="col-lg-3 people-col">
             <div class="col-12 people-wrap">
               <?php foreach ($followedActive as $person) : ?>
               <div class="media">
                 <img class="d-flex align-self-center mr-3" src="/assets/img/<?=$person['avatar']?>" alt="Generic placeholder image">
                 <div class="media-body align-self-center">
                   <h6 class="mt-0"><?=$person['fname']?> <?=$person['lname']?><span class="date-ago float-right <?php if ($person['active'] == 'active') : ?>online<?php endif; ?>"><?php if ($person['active'] != 'active') : ?><?=$person['active']?><?php endif; ?></span></h6>
                 </div>
               </div>
             <?php endforeach; ?>
             </div>
             <div class="col-12 people-search no-padding">
               <i class="material-icons">search</i>
               <input type="text" class="form-control" placeholder="Search...">
             </div>
           </div>
         </div>
    </div>
    <footer>
      <hr>
      <p class="text-center">Copyright &copy; Adrian Kluska 2017</p>
    </footer>

    <!-- Optional JavaScript -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <script src="/assets/js/jquery.slim.min.js"></script>
    <script src="/assets/js/popper.min.js"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta/js/bootstrap.min.js" integrity="sha384-h0AbiXch4ZDo7tp9hKZ4TsHbi047NrKGLO3SEJAg45jXxnGIfYzk4Si90RDIqNm1" crossorigin="anonymous"></script>

    <script type="text/javascript">
        $('.showReactionOpions').popover({
            html: true,
        });

        $('.send-comment').on('click', function(e){
          e.preventDefault();
          $(this).closest('.feed-box').addClass('dimmed');
        });

        $('.close-comment').on('click', function(e){
          e.preventDefault();
          $(this).closest('.feed-box').removeClass('dimmed');
        });

           function replaceEmoticons(text) {
             var emoticons = {
               ':)'  : 'smiling-face.png',
               ':D'  : 'grinning-face.png',
               '3:)'  : 'smiling-face-with-horns.png',
               '*.*' : 'astonished-face.png',
               ':O' : 'face-with-open-mouth.png',
               ':tearsofjoy:' : 'face-with-tears-of-joy.png',
               ':S' : 'confused-face.png',
               ';)' : 'winking-face.png'
             }, url = "/assets/emojis/", patterns = [],
                metachars = /[[\]{}()*+?.\\|^$\-,&#\s]/g;

             // build a regex pattern for each defined property
             for (var i in emoticons) {
               if (emoticons.hasOwnProperty(i)){ // escape metacharacters
                 patterns.push('('+i.replace(metachars, "\\$&")+')');
               }
             }

             // build the regular expression and replace
             return text.replace(new RegExp(patterns.join('|'),'g'), function (match) {
               return typeof emoticons[match] != 'undefined' ?
                      '<img class="emoji" src="'+url+emoticons[match]+'"/>' :
                      match;
             });
           }

        $('p').each(function(index, value){
          return $(this).html(replaceEmoticons($(this).html()));
        });

        $('.videoEmbedded').height($('.videoEmbedded').width() * (9/16));

        $(window).on('resize', function(){
          $('.videoEmbedded').height($('.videoEmbedded').width() * (9/16));
          setSidebar();
        });

        $('#notificationDropdown').on('click', function(e){
          e.preventDefault();
          $(this).removeClass('has-notify');
        });

        var isSafari = /constructor/i.test(function HTMLElementConstructor() {});
        if (!isSafari) {
          $('.people-col, .feed-box-comments').on('mouseover', function(){
            $('body').css({overflow: 'hidden', paddingRight: '17px'});
            $('.people-col').css({marginRight: '17px'});
            $('.top-bar').css({paddingRight: '62px'});
          });

          $('.people-col, .feed-box-comments').on('mouseout', function(){
            $('body').css({overflow: 'auto', paddingRight: 0});
            $('.people-col').css({marginRight: '0'});
            $('.top-bar').css({paddingRight: '45px'});
          });
        }
        else {
          $('.people-col, .feed-box-comments').on('mouseover', function(){
            $('body').css({overflow: 'hidden'});
          });

          $('.people-col, .feed-box-comments').on('mouseout', function(){
            $('body').css({overflow: 'auto'});
          });

          $('.people-col .people-wrap').css({width: '100%'});
        }

        function setSidebar() {
          var peopleRight = ($(window).innerWidth() * 0.25) - ($('.people-col').width() * 1.5);
          peopleRight < 0 ? peopleRight = 0 : true;
          $('.people-col').css({right: peopleRight});
        }

        setSidebar();

      function returnUrl(elem) {
        var url = elem.dataset.href;

        window.location = url;
      }

      $('.comment-submit').on('click', function(){
        $(this).closest('form').submit();
      });

      $('.showReactionOpions').on('click', function(e) {
        e.preventDefault();
      });
    </script>
  </body>
</html>
